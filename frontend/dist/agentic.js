import { buildEndpointUrl, setStatus, nowTime, typewriterEffect, copyToClipboard, fetchAuthTokenFromBackend, getAuthToken, clearTokenCache, } from "./common.js";
import { bindClampedNumberInput, createRateLimitController, loadSettingsFromStorage, saveSettingsToStorage, } from "./shared/page_helpers.js";
...
function renderAgenticProgress(container, { sources, searchCount, queryType }) {
    ...
    if (!hasSources && !hasSearchCount && !hasQueryType)
        return;
        ...
        if (pre) {
            if (thinking) {
                ...
                if (useTypewriter && text) {
                    typewriterEffect(messages, pre, text);
                }
                else {
...
titleK = document.getElementById("titleK");
chunkK = document.getElementById("chunkK");
queryExpandK = document.getElementById("queryExpandK");
